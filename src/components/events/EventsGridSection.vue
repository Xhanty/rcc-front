  ...
  <section class="px-6 max-w-max-width mx-auto pb-20 z-10 relative">
    <transition name="fade" mode="out-in">
      <div v-if="hasVisibleEvents" class="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        >
          <div class="aspect-video relative overflow-hidden flex-grow min-h-[250px]">
            <img 
              :alt="filteredFeaturedEvent.title" 
            ...
            <div class="absolute bottom-0 left-0 p-6 md:p-10 text-white">
              <span class="inline-block px-4 py-1 bg-secondary text-on-secondary-fixed rounded-full text-xs font-bold mb-4">Evento Principal</span>
          ...
          <div class="p-6 md:p-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-base">
            ...
            <button class="bg-primary text-on-primary px-8 py-3 rounded-xl font-title-md shadow-lg active:scale-95 transition-all mt-4 md:mt-0 cursor-pointer">
              Inscribirse Ahora
            </button>
          </div>
        </div>
        ...
        <!-- Secondary Cards -->
        <div 
          v-for="event in filteredSecondaryEvents" 
          :key="event.id"
        ...
        >
          ...
          <div class="p-6 md:p-8 flex flex-col flex-grow">
            ...
            <p class="font-label-sm text-label-sm text-secondary mb-4 uppercase tracking-widest">{{ event.dateLabel }}</p>
            <p class="font-body-md text-body-md text-on-surface-variant mb-6 flex-grow">
              {{ event.desc }}
            </p>
            <div class="mt-auto pt-4 faded-rule">
              <button class="w-full mt-2 border border-secondary text-secondary py-3 rounded-xl font-title-md hover:bg-secondary/5 transition-all cursor-pointer active:scale-95">
                Ver Detalles
              </button>
            </div>
          </div>
        ...
        <div 
          v-if="filteredOtherMeetings.length > 0" 
        ...
        >
          ...
          <div class="space-y-4">
            <div 
              v-for="(meeting, index) in filteredOtherMeetings" 
              :key="meeting.id"
              :class="['flex items-center justify-between py-6', { 'faded-rule': index !== filteredOtherMeetings.length - 1 }]"
            >
              <div class="flex items-center gap-6">
                <!-- Date column -->
                <div class="w-14 h-14 flex flex-col items-center justify-center bg-surface-container-high text-primary rounded-xl flex-shrink-0">
      ...
      <div v-else class="text-center py-20 glass-card rounded-xl max-w-xl mx-auto p-10">
        ...
        <h3 class="font-headline-lg text-headline-lg text-primary mt-4">Sin Encuentros</h3>
        <p class="font-body-md text-body-md text-on-surface-variant mt-2">
          No hay encuentros programados para el mes de {{ selectedMonth }}. Te invitamos a revisar los otros meses del calendario.
        </p>
        <button @click="$emit('reset-filter')" class="mt-6 bg-primary text-on-primary px-8 py-2 rounded-full font-body-md hover:bg-primary-container transition-all active:scale-95 cursor-pointer">
          Ver Todo
        </button>
      </div>
    </transition>
